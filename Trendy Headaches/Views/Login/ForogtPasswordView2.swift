...
struct ForgotPasswordView2: View {
    ...
    private var correct: Bool {
        ...
        let normDBAnswer = Database.normalize(SA)
        return normAnswer == normDBAnswer
    }
    ...
    var body: some View {
        NavigationStack {
            ZStack {
                ...
                .task {
                    userID = await Database.shared.userFromEmail(email: email)
                    SQ = (try? await Database.shared.getSingleVal(userId: userID ?? -1, col: "security_question")) ?? ""
                    SA = (try? await Database.shared.getSingleVal(userId: userID ?? -1, col: "security_answer")) ?? ""
                }
            }
        }
    }
...
#Preview {
    ForgotPasswordView2(email: "testtest@test.com2")
}
